<template lang="pug">
h1(class="h-[62px] bg-white text-[#060C34] text-[32px] font-bold py-2 px-8 border-b border-[#E7E7E7] hidden xl:block ml-[260px]") {{ t('userCenter.withdrawalRecord.title') }}
        ...
        table(class="w-full text-xs text-center bg-white rounded-[10px] overflow-hidden")
          thead(class="bg-[#F6E27B] text-[#060C34] font-bold")
            tr
              ...
              th(class="py-3 w-[62px]") {{ t('userCenter.common.headers.requestTime') }}
              th(class="py-3 w-[80px]") {{ t('userCenter.common.headers.paidAmountHeader') }}
          ...
          tbody
            tr(v-for="r in records" :key="r.id" class="border-b border-[#E5E7EB] cursor-pointer" @click="openDetail(r)")
              td(class="py-3 text-[#060C34]") {{ r.transaction_number }}
              td(class="py-3 text-[#060C34]") {{ dayjs.unix(r.request_time).format('YYYY-MM-DD HH:mm:ss') }}
              td(class="py-3 font-bold text-[#060C34]") {{ r.paid_amount ? formatAmount(r.paid_amount) : '-' }}
              td(class="py-3 font-bold" :class="r.status === 'Approved' ? 'text-[#1DBF73]' : r.status === 'Rejected' ? 'text-[#E11D48]' : 'text-[#d19200]'") {{ r.status }}
        ...
        div(class="min-w-[1000px]")
          div(class="bg-[#F6E27B] text-[#060C34] font-bold rounded-t-[10px]")
            div(class="flex justify-between items-center px-4 py-3 text-sm text-center")
              p(class="w-[120px]") {{ t('userCenter.common.headers.transactionNo') }}
              p(class="w-[140px]") {{ t('userCenter.common.headers.requestTime') }}
              p(class="w-[132px]") {{ t('userCenter.common.headers.paidAmountHeader') }}
              p(class="w-[70px]") {{ t('userCenter.common.headers.status') }}
              p(class="w-[140px]") {{ t('userCenter.common.headers.requestAmountHeader') }}
              p(class="w-[104px]") {{ t('userCenter.common.headers.bankName') }}
              p(class="w-[111px]") {{ t('userCenter.common.headers.paidDate') }}
              p(class="w-[69px]") {{ t('userCenter.common.headers.remarkHeader') }}
          div(v-for="r in records" :key="r.id" class="flex justify-between items-center px-4 py-3 text-sm text-center text-[#060C34] bg-white border-b border-[#E5E7EB]")
            div(class="w-[120px]") {{ r.transaction_number }}
            p(class="w-[140px]") {{ dayjs.unix(r.request_time).format('YYYY-MM-DD HH:mm:ss') }}
            p(class="w-[132px]") {{ r.paid_amount ? formatAmount(r.paid_amount) : '-' }}
            p(class="w-[70px] font-bold" :class="r.status === 'Approved' ? 'text-[#1DBF73]' : r.status === 'Rejected' ? 'text-[#E11D48]' : 'text-[#d19200]'") {{ r.status }}
            p(class="w-[140px]") {{ r.request_amount ? formatAmount(r.request_amount) : '-' }}
            p(class="w-[104px]") {{ r.bank_name || '-' }}
            p(class="w-[111px]") {{ r.paid_date === 0 ? '-' : dayjs.unix(r.paid_date).format('YYYY-MM-DD HH:mm:ss') }}
            p(class="w-[69px]") {{ r.remark || '-' }}
    ...
    div(class="fixed bottom-0 left-0 right-0 z-40 xl:left-[240px] xl:bottom-4 justify-center")
      div(class="w-full xl:max-w-[1190px] mx-auto px-4 bg-[#0A1140] text-[#F6E27B] xl:rounded")
        div(class="flex items-center justify-between h-12")
          p {{ t('userCenter.common.footer.totalWithdrawalAmount') }}
    ...
    div(v-if="showDetail" class="fixed inset-0 z-50 flex items-center justify-center")
      ...
      div(class="relative bg-white rounded-[10px] w-[90%] max-w-[560px] px-6 py-4 text-[#060C34]")
        ...
        div(class="grid grid-cols-1 gap-1 text-[#060C34]")
          p(class="text-sm") {{ t('userCenter.common.headers.paidAmountHeader') }}：{{ detail.paid_amount ? formatAmount(detail.paid_amount) : '---' }}
          p(class="text-sm") {{ t('userCenter.common.headers.requestAmountHeader') }}：{{ detail.request_amount ? formatAmount(detail.request_amount) : '---' }}
          p(class="text-sm") {{ t('userCenter.common.headers.bankName') }}：{{ detail.bank_name || '---' }}
          p(class="text-sm") {{ t('userCenter.common.headers.paidDate') }}： {{ detail.paid_date === 0 ? '---' : dayjs.unix(detail.paid_date).format('YYYY-MM-DD HH:mm:ss') }}
          p(class="text-sm") {{ t('userCenter.common.headers.remarkHeader') }}： {{ detail.remark || '---' }}
  ...
  if (data.data.length) {
    data.data.forEach((r) => {
      if (r.status === 'Approved') {
        totalAmount.value += Number(r.paid_amount || 0)
      }
    })
  }
...
async function refreshCurrentPage() {
  ...
  if (startIndex < 0 || startIndex >= records.value.length) return
